function [xdot,Thrust_] = otter(x,n,mp,rp,V_c,beta_c)
...
for i = 1:1:2
    ...
    if n(i) > 0
        ...
        %      Thrust(i) = (h/T_thrust) * ((k_pos * n(i)*abs(n(i))) - output_prev(i)) + output_prev(i);
    else
        Thrust(i) = k_neg * n(i)*abs(n(i));    % aft force (N)
        %       Thrust(i) = (h/T_thrust) * ((k_neg * n(i)*abs(n(i))) - output_prev(i)) + output_prev(i);
    end
end

Thrust_= [Thrust(1),Thrust(2)];
...
xdot = [...
    M \ ( tau + [Xh Yh Zh Kh Mh Nh]' - C * nu_r - G * eta - g_0)
    Rzyx * nu1
    Tzyx * nu2 ];
end
...
C = zeros(6,6);   
C(1:3,4:6) = -Smtrx(dt_dnu1);
C(4:6,1:3) = -Smtrx(dt_dnu1);
C(4:6,4:6) = -Smtrx(dt_dnu2);
...
%DATA = [B/2T  C_D]
DATA1 = [0.0109,0.1766,0.3530,0.4519,0.4728,0.4929,0.4933,0.5585,0.6464,0.8336,0.9880,1.3081,1.6392,1.8600,2.3129,2.6000,3.0088,3.4508, 3.7379,4.0031 ];
DATA2 = [1.9661,1.9657,1.8976,1.7872,1.5837,1.2786,1.2108,1.0836,0.9986,0.8796,0.8284,0.7599,0.6914,0.6571,0.6307,0.5962,0.5868,0.5859,0.5599,0.5593 ];
CY_2D =interp1 ( DATA1, DATA2 ,B / (2 * T) );
end
